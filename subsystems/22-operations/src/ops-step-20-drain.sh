...
# drain の入力は step15 が出力した失敗ファイル。MQ は既定 mock(M)。RabbitMQ 実投入は OPS_MQ_MODE=R
...
"$DRIVER" step20 \
    "${OPS_DRAIN_SOURCE:-$WORK/ad-failed-$BDATE.dat}" "${OPS_DRAIN_MAX:-10000}" "${OPS_MQ_MODE:-M}" \
